<?php
// SELECT *, WEEK(tgl, 1) - WEEK(DATE_SUB(tgl, INTERVAL DAYOFMONTH(tgl)-1 DAY), 1) + 1 AS minggu FROM tb_jrnl WHERE kd_staf = 11014 ORDER BY tgl ASC;


$pr_jrnl = db_Proses($pdo_conn, "SELECT *, WEEK(tgl, 1) - WEEK(DATE_SUB(tgl, INTERVAL DAYOFMONTH(tgl)-1 DAY), 1) + 1 AS minggu FROM tb_jrnl WHERE kd_staf = ? ORDER BY tgl ASC;", [$id_staf]);
$alt = db_Proses($pdo_conn, "SELECT WEEK(tgl, 1) - WEEK(DATE_SUB(tgl, INTERVAL DAYOFMONTH(tgl)-1 DAY), 1) + 1 AS minggu, COUNT(tgl) AS alt FROM tb_jrnl WHERE kd_staf = ? GROUP BY minggu ORDER BY alt DESC;", [$id_staf])->fetch(PDO::FETCH_ASSOC);
$d_jrnal = $pr_jrnl->fetch(PDO::FETCH_ASSOC);

$thn = $d_jrnal['tgl'] != '' ? date('Y', strtotime($d_jrnal['tgl'])) : '';
...
if ($cvr == '1'):
	...
	$imgW = 70;
	$pageWidth = ($orien == 'L') ? $cfg['size'][1] : $cfg['size'][0];
	$x = ($pageWidth - $imgW) / 2;
	$pdf->Image('../images/logo_pt.png', $x, $yImg, $imgW, 70); // (file,x,y,width,height)

	function dInfo($pdf, $nama, $info, $orien)
	{
	...
	$dkls = '';
	if (is_array($pkls)) {
		$kls_vals = array_values($pkls);
		if (count($kls_vals) > 1) {
			$last = array_pop($kls_vals);
			$dkls = trim(implode(', ', $kls_vals) . ' & ' . $last);
		} else {
			$dkls = trim($kls_vals[0]);
		}
	} else {
		$dkls = trim((string)$pkls);
	}

	if ($orien == 'L') {
		$pdf->SetXY(10, 120);
		$pdf->Cell($jrk_list, 7, 'Daftar Hadir Siswa', 0, 0, 'C');
		$pdf->Cell($jrk_list, 7, 'Daftar Nilai Siswa', 0, 0, 'C');
		$pdf->Cell($jrk_list, 7, 'Jurnal Guru Mengajar', 0, 1, 'C');
		$pdf->Cell(0, 7, '', 0, 1);
		$pdf->Cell(0, 7, '', 0, 1);
	} else {
		if ($_POST['kertas'] == 'a4') {
		...
		// $pdf->SetXY(0, 160);
		$pdf->Cell(0, 7, 'Daftar Hadir Siswa', 0, 2, 'C');
		$pdf->Cell(0, 7, 'Daftar Nilai Siswa', 0, 2, 'C');
		$pdf->Cell(0, 7, 'Jurnal Guru Mengajar', 0, 2, 'C');
		$pdf->Cell(0, 7, '', 0, 1);
...
if (!empty($pkls)):
	...
	foreach ($bln_arr as $i => $bln) {
		...
		foreach ($pkls as $kls) {
			...
			// Tampilan Halaman
			foreach ($kl as $tm) {
				$cek_kls = db_Proses($pdo_conn, "SELECT COUNT(*) AS jml FROM tb_dsis WHERE kls =?", [$kls])->fetch(PDO::FETCH_ASSOC);
				...
				if ($cek_kls['jml'] != 0) {
					...
					if ($nipd_sis->rowCount() == 0) {
						continue;
					}
					$nipd_sis = $nipd_sis->fetch(PDO::FETCH_ASSOC);
				...
				$jml_l = $jml_p = 0;
				while ($row = $jml_k->fetch(PDO::FETCH_ASSOC)) {
					if ($row['jk'] == 'L') $jml_l++;
				...
				while ($dt_sis = $siswa->fetch(PDO::FETCH_ASSOC)) {
					...
					// Daftar Nilai
					$pdf->Cell($h_dn_no, $t_tbl, '60', 1, 0, 'C');
